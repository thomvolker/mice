...
set.seed(123)
svd <- .norm.draw.new(y, ry, x, ls.meth = "svd")
set.seed(123)
ridge <- .norm.draw.new(y, ry, x, ls.meth = "ridge")
set.seed(123)
qr <- .norm.draw.new(y, ry, x, ls.meth = "qr")

# tests for test1
test_that("Estimates are equal", {
  expect_equal(svd$coef, matrix(qr$coef))
  # expect_equal(svd$beta, matrix(qr$beta)) # svd uses different decomposition, so draws are not equal
...
expect_warning(imp.qr <- mice(mammalsleep[, -1], ls.meth = "qr", meth = "norm.new", seed = 123, print = FALSE, use.matcher = TRUE))
expect_warning(imp.svd <- mice(mammalsleep[, -1], ls.meth = "svd", meth = "norm.new", seed = 123, print = FALSE, use.matcher = TRUE))
expect_warning(imp.ridge <- mice(mammalsleep[, -1], ls.meth = "ridge", meth = "norm.new", seed = 123, print = FALSE, use.matcher = TRUE))

test_that("Imputations are equal", {
  #expect_equal(imp.qr$imp, imp.svd$imp) # not equal anymore because svd draws coefficients differently
...
expect_warning(imp.qr <- mice(mammalsleep, meth = "norm.new", ls.meth = "qr", seed = 123, print = FALSE))
expect_warning(imp.svd <- mice(mammalsleep, meth = "norm.new", ls.meth = "svd", seed = 123, print = FALSE))
...
test_that("Returns requested length", {
  expect_equal(length(mice.impute.norm.new(y, ry, x)), sum(!ry))
  expect_equal(length(mice.impute.norm.new(y, ry, x, wy = wy1)), sum(wy1))
  expect_equal(length(mice.impute.norm.new(y, ry, x, wy = wy2)), sum(wy2))
  expect_equal(length(mice.impute.norm.new(y, ry, x, wy = wy3)), sum(wy3))
  expect_equal(length(mice.impute.norm.new(y, ry, x, wy = wy4)), sum(wy4))
})
